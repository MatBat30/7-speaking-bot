// ==UserScript==
// @name     	7Speaking-bot-with-error
// @namespace	https://github.com/MatBat30
// @version  	2.1
// @description  Automates tasks on 7Speaking website with a 15% error rate
// @author   	Shura
...
(async () => {
    ...
    function shouldMakeError() {
        return Math.random() < 0.15; // 15% error rate
    }
    ...
    async function completeQuiz() {
        ...
        if (shouldMakeError()) {
            console.log("Introducing an error...");
            const answers = Array.from(document.querySelectorAll('.question__customLabel'));
            const incorrectAnswers = answers.filter(a => a.innerText.trim() !== answer.trim());
            if (incorrectAnswers.length > 0) {
                answer = incorrectAnswers[Math.floor(Math.random() * incorrectAnswers.length)].innerText.trim();
            }
        }
    ...
    async function completeExam() {
        ...
        if (answer === null || answer === undefined) {
            ...
            if (shouldMakeError() && typeof answer === 'object') {
                console.log("Introducing an error...");
                if (Array.isArray(answer)) {
                    answer = answer.map(a => a === 1 ? 0 : 1); // Simple toggle for error
                } else {
                    for (const key in answer) {
                        answer[key] = answer[key] === 1 ? 0 : 1; // Simple toggle for error
                    }
                }
            }
